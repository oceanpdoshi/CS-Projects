...
{
	//Base case 1 - given in function description
	if (n2 <= 0) return 1;

	//Base case 2 - have reached the end of a1
	if (n1 <= 1) {
		//They have to match AND you have to be at the last element of the 2nd list.
		if (a1[0] == a2[0] && n2 == 1)
			return 1;
		else 
			return 0;
	...
	//Recursive calls, does this work? What about undefined behavior...
	if (a1[0] == a2[0])
...
{
	//Given function description. Base Case?
	if (n <= 1)
		return;
	...
	int firstNotLess = n;
	int firstGreat = n;
	separate(a, n, a[n/2], firstNotLess, firstGreat);
	//order the left side of the array (not including separator)
	order(a,firstNotLess);
	//order the right side of the array (including separator)
	order(a + firstNotLess, n - firstGreat + 1);
